...
def fit_lmk2d(target_img, target_2d_lmks, model_fname, lmk_face_idx, lmk_b_coords, weights, visualize):
    ...
    tf_trans = tf.Variable(np.zeros((1, 3)), name="trans",
                           dtype=tf.float64, trainable=True)
    tf_rot = tf.Variable(np.zeros((1, 3)), name="rot",
                         dtype=tf.float64, trainable=True)
    tf_pose = tf.Variable(np.zeros((1, 12)), name="pose",
                          dtype=tf.float64, trainable=True)
    tf_shape = tf.Variable(np.zeros((1, 300)), name="shape",
                           dtype=tf.float64, trainable=True)
    tf_exp = tf.Variable(np.zeros((1, 100)), name="expression",
                         dtype=tf.float64, trainable=True)
    ...
    with tf.Session(config=config) as session:
        ...
        lmks_3d = tf_get_model_lmks(
            tf_model, smpl.f, lmk_face_idx, lmk_b_coords)

        s2d = np.mean(np.linalg.norm(target_2d_lmks -
                                     np.mean(target_2d_lmks, axis=0), axis=1))
        s3d = tf.reduce_mean(tf.sqrt(tf.reduce_sum(
            tf.square(lmks_3d-tf.reduce_mean(lmks_3d, axis=0))[:, :2], axis=1)))
        ...
        neck_pose_reg = weights['neck_pose'] * \
            tf.reduce_sum(tf.square(tf_pose[:, :3]))
        jaw_pose_reg = weights['jaw_pose'] * \
            tf.reduce_sum(tf.square(tf_pose[:, 3:6]))
        eyeballs_pose_reg = weights['eyeballs_pose'] * \
            tf.reduce_sum(tf.square(tf_pose[:, 6:]))
        ...
        if visualize:
            def on_step(verts, scale, faces, target_img, target_lmks, opt_lmks, lmk_dist=0.0, shape_reg=0.0, exp_reg=0.0, neck_pose_reg=0.0, jaw_pose_reg=0.0, eyeballs_pose_reg=0.0):
                ...
                plt_target_lmks[:, 1] = target_img.shape[0] - \
                    plt_target_lmks[:, 1]
                for (x, y) in plt_target_lmks:
                    cv2.circle(target_img, (int(x), int(y)),
                               4, (0, 0, 255), -1)
                ...
                if sys.version_info >= (3, 0):
                    rendered_img = render_mesh(
                        Mesh(scale*verts, faces), height=target_img.shape[0], width=target_img.shape[1])
                    for (x, y) in plt_opt_lmks:
                        cv2.circle(rendered_img, (int(x), int(y)),
                                   4, (255, 0, 0), -1)
        ...
        loss = lmk_dist
        optimizer = scipy_pt(loss=loss, var_list=vars,
                             method='L-BFGS-B', options={'disp': 1, 'ftol': 5e-6})
        optimizer.minimize(session, fetches=[tf_model, tf_scale, tf.constant(smpl.f), tf.constant(
            target_img), tf.constant(target_2d_lmks), lmks_proj_2d], loss_callback=on_step)

        print('Optimize model parameters')
        vars = [tf_scale, tf_trans[:2], tf_rot, tf_pose, tf_shape, tf_exp]
        loss = lmk_dist + shape_reg + exp_reg + \
            neck_pose_reg + jaw_pose_reg + eyeballs_pose_reg

        optimizer = scipy_pt(loss=loss, var_list=vars,
                             method='L-BFGS-B', options={'disp': 0, 'ftol': 1e-7})
...
def run_2d_lmk_fitting(model_fname, flame_lmk_path, texture_mapping, target_img_path, target_lmk_path, out_path, visualize):
    ...
    result_mesh, result_values = fit_lmk2d(
        target_img, lmk_2d, model_fname, lmk_face_idx, lmk_b_coords, weights, visualize)

    if sys.version_info >= (3, 0):
        texture_data = np.load(
            texture_mapping, allow_pickle=True, encoding='latin1').item()
    else:
    ...
    elif 'female_model.pkl' in model_fname:
        result_values = np.append(np.array([1]), result_values)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--model_fname', default='./models/generic_model.pkl', help='Path of the FLAME model')
    # Path of the landamrk embedding file into the FLAME surface
    parser.add_argument('--flame_lmk_path', default='./data/flame_static_embedding.pkl',
                        help='Path of the landamrk embedding file into the FLAME surface')
    # Pre-computed texture mapping for FLAME topology meshes
    parser.add_argument('--texture_mapping', default='./data/texture_data.npy',
                        help='pre-computed FLAME texture mapping')
    # Target image (used for visualization only)
    parser.add_argument(
        '--target_img_path', default='./data/imgHQ00088.jpeg', help='Path of the target image')
    ...
    parser.add_argument('--target_lmk_path', default='./data/imgHQ00088_lmks.npy',
                        help='2D landmark file that should be fitted (landmarks must be corresponding with the defined FLAME landmarks)')
    # Output path
    parser.add_argument('--out_path', default='./results',
                        help='Path of the fitting output')
    # Visualize fitting
    parser.add_argument('--visualize', default='True',
                        help='Visualize fitting progress and final fitting result')
